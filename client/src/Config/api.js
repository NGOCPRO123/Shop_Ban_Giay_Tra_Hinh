...
// Phương thức API mới cho OTP
export const requestSendOTP = async (data) => {
    const res = await request.post('/api/payment/sendotp', data);
    return res.data;
};

export const requestVerifyOTP = async (paymentData) => {
    const res = await request.post('/api/payment/verifyotp', paymentData);
    return res.data;
};
